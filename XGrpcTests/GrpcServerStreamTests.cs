...
public class  GrpcServerStreamTests
{
    ...
    public async Task GetStream()
    {
        ...
        var result = await calculator.GetRange(0, 10, TimeSpan.FromSeconds(0.1)).ToListAsyncInternal();
    ...
    public async Task GetStreamTimeout()
    {
        ...
        calculator.MaxCallDuration = TimeSpan.FromSeconds(5);
        var ex = await Assert.ThrowsAsync<RpcException>(async () => { await calculator.GetRange(0, 10, TimeSpan.FromSeconds(1)).ToListAsyncInternal(); });
